...
public final class CodeDefenseRuntimeFactory {
 ...
 CodeDefenseRuntime create(AiProvider provider,ObjectMapper mapper,CodexRuntimeConfig config,InterviewConfig interviewConfig,UserInput input,InterviewOutput output){
  ...
  InterviewRunner interview=new InterviewEngine(evaluator,input,output,new InterviewScorer(),new ReadinessClassifier(),interviewConfig);return new CodeDefenseRuntime(analyzer,interview);
 }
}
