...
public final class SecretRedactor {
    private static final Pattern ASSIGNMENT = Pattern.compile("(?im)(\\b(?:password|passwd|secret|apiKey|api_key|api-key|token|accessToken|access_token|clientSecret|client_secret|authorization)\\b\\s*[:=]\\s*)([^\\r\\n]+)");
    ...
    public RedactionResult redact(String content) {
        int[] count = {0};
        String bearer = replace(BEARER, content, count);
    ...
    private String replace(Pattern pattern, String input, int[] count) {
        ...
        while (matcher.find()) { count[0]++; matcher.appendReplacement(out, Matcher.quoteReplacement(matcher.group(1) + "[REDACTED]")); }
