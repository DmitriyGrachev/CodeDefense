...
public final class SecretRedactor {
    ...
    private static final Pattern QUOTED_ASSIGNMENT = Pattern.compile("(?im)(\\b(?:" + KEYS + ")\\b\\s*[:=]\\s*)([\"'])((?:\\\\.|[^\"'\\\\])*)([\"'])");
    ...
    public RedactionResult redact(String content) {
        ...
        String quoted = replace(QUOTED_ASSIGNMENT, bearer, count);
        return new RedactionResult(replace(ASSIGNMENT, quoted, count), count[0]);
    }
    private String replace(Pattern pattern, String input, int[] count) {
        ...
        while (matcher.find()) { count[0]++; String suffix = matcher.groupCount() == 3 ? matcher.group(3) : matcher.groupCount() == 4 ? matcher.group(2) + "[REDACTED]" + matcher.group(4) : "[REDACTED]"; matcher.appendReplacement(out, Matcher.quoteReplacement(matcher.group(1) + (matcher.groupCount() == 4 ? suffix : "[REDACTED]" + suffix))); }
