...
public final class ConsoleInterviewOutput implements InterviewOutput {
 ...
 public void renderSummary(InterviewSession s){output.printf("%nTechnical defense completed.%n%nQuestion scores%n");for(int index=0;index<s.results().size();index++)output.printf("%d. %d/100%n",index+1,s.results().get(index).finalScore());output.printf("%nOverall score: %d/100%nReadiness: %s%n%n",s.overallScore(),safe(s.readiness().displayName()));output.println("Educational signal only. This is not approval to merge or deploy.");output.println("Report generation will be connected in Iteration 7.");flush();}
